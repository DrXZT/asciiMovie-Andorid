...
import com.asciimovie.drxzt.asciimovie.R;
...
public class AlxGifHelper {
     ...
     * @param gifView
     */
    public static void displayImage(final String url,final GifImageView gifView, int displayWidth){
        //首先查询一下这个gif是否已被缓存
       ...
       Log.i("AlexGIF","gif图片的缓存路径是"+path);
        ...
        if(memoryCache!=null && memoryCache.get(url)!=null){//如果以前有别的imageView加载过
           Log.i("AlexGIF","以前有别的ImageView申请加载过该gif"+url);
            ...
            memoryCache.get(url).add(new ProgressViews(imageViewWait,displayWidth));
            return;
        }
        ...
        startDownLoad(url, new File(cacheFile.getAbsolutePath()+".tmp"), new DownLoadTask() {
            ...
            public void onLoading(long total, long current) {
                int progress = 0;
                //得到要下载文件的大小，是通过http报文的header的Content-Length获得的，如果获取不到就是-
               Log.i("AlexGIF","正在加载");
            }
            @Override
            public void onSuccess(File file) {
                ...
                ArrayList<AlxGifHelper.ProgressViews> viewArr = memoryCache.get(url);
                if(viewArr==null || viewArr.size()==0)return;
                for(AlxGifHelper.ProgressViews ws:viewArr){//遍历所有的进度条和imageView，同时修改所有请求同一个url的进度
                    //显示imageView
